...
import { getAvailableOAuthProviders } from '../../lib/authApi';
...
export function LoginForm({ onSwitchToRegister }: LoginFormProps) {
  ...
  useEffect(() => {
    // Fetch available OAuth providers on component mount
  ...
  return (
    <div className="w-full max-w-md p-8 space-y-6 bg-gray-900 border border-gray-800 rounded-lg">
      <div className="text-center">
        <h2 className="text-2xl font-bold text-white">Sign In to TriTerm</h2>
        <p className="mt-2 text-sm text-gray-400">Enter your credentials to access your terminals</p>
      </div>
        ...
        <>
          ...
          <div
            className={`grid gap-3 ${
              availableProviders.length === 1 ? 'grid-cols-1' : 'grid-cols-2'
            }`}
          >
            ...
            {availableProviders.includes('github') && (
              <Button
                ...
                disabled={loading}
                onClick={() => {
                  window.location.href = `${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/api/auth/oauth/github`;
                }}
                className="bg-gray-800 border-gray-700 text-white hover:bg-gray-700"
              >
